...
export const About: React.FC<{ scrollHandler: (scrollTo: string) => () => void; isFrozen: boolean }> = ({ scrollHandler, isFrozen }): JSX.Element => (
	<>
		...
		<div>
			...
			I've done some technical work in the area of embedded AI for musical performance and instrument design, but also a lot of writing on how we can approach technology development
			(particularly in creative contexts, e.g. new instrument making) from a more critical and reflective perspective. You can check out my research{' '}
			<a onClick={scrollHandler('research')}>here</a>.
			<br />
			<br /> Before joining my PhD programme (CDT in AI and Music), I got a Bachelor degree in <span className={'h-yellow bounce' + (isFrozen ? ' frozen' : '')}>Physics</span> (Universidad
			Autónoma de Madrid) and a Master's in <span className={'h-yellow bounce' + (isFrozen ? ' frozen' : '')}>Sound and Music Computing</span> (Music Technology Group, Universitat Pompeu Fabra).
			In my spare time I mostly work on my Raspberry Pi self-hosted servers, and I also make candles and play football.
			<br />
			<br />
			my{' '}
